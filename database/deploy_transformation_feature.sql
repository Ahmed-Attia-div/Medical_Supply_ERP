-- ======================================
-- INVENTORY TRANSFORMATION - DEPLOYMENT SCRIPT
-- ======================================
-- Run this in Supabase SQL Editor if not already deployed
-- ======================================

-- Step 1: Create the inventory_transformations table
...
DROP POLICY IF EXISTS "Allow read access to authenticated users" ON public.inventory_transformations;
CREATE POLICY "Allow read access to authenticated users"
    ON public.inventory_transformations FOR SELECT
    USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow insert access to authenticated users" ON public.inventory_transformations;
...
SELECT EXISTS (
    ...
    AND table_name = 'inventory_transformations'
) AS table_exists;

-- Check if function exists
SELECT EXISTS (
    SELECT FROM pg_proc 
    WHERE proname = 'transform_inventory_item'
) AS function_exists;

-- Check RLS policies
SELECT schemaname, tablename, policyname 
FROM pg_policies 
WHERE tablename = 'inventory_transformations';

-- ======================================
-- SUCCESS MESSAGE
-- ======================================
-- If all checks show TRUE, the transformation feature is ready! ✅
